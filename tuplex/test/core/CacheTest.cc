...
TEST_F(CacheTest, SimpleCSVLoad) {
    using namespace std;
    auto fileURI = URI("CacheTest.SimpleCSVLoad.csv");
...
TEST_F(CacheTest, LogicalOptCSVLoad) {
    using namespace std;
    auto fileURI = URI("CacheTest.LogicalOptCSVLoad.csv");
    auto opt = microTestOptions();
...
TEST_F(CacheTest, NullValueOptIf) {
    ...
    stringToFile(URI("CacheTest.NullValueOptIf.csv"), ss.str());

    // => cache materializes both normal and exceptional case in memory
    // => this can be then used to speed up processing!
    auto& ds_cached = c.csv("CacheTest.NullValueOptIf.csv").cache();
...
TEST_F(CacheTest, NullValueOptIfAlt) {
    ...
    for(int i = 0; i < 10; ++i)
        ss<<"10,20\n";
    ss<<",20\n"; // single non-conforming row, which gets cached separately!
    stringToFile(URI("CacheTest.NullValueOptIfAlt.csv"), ss.str());

    // => cache materializes both normal and exceptional case in memory
    // => this can be then used to speed up processing!
    auto& ds_cached = c.csv("CacheTest.NullValueOptIfAlt.csv").cache();
