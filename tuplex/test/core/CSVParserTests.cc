...
TEST(CSVDataSet, ParseWithMultiplePartitions) {
    ...
    // write temp file
    FILE *fp = fopen("CSVDataSet.ParseWithMultiplePartitions.csv", "w");
    ASSERT_TRUE(fp);
    ...
    auto v = c.csv("CSVDataSet.ParseWithMultiplePartitions.csv").mapColumn("columnB", UDF("lambda x: x * x")).collectAsVector();
...
TEST(CSVDataSet, ParseWithMultiplePartitionsLimit) {
    ...
    // write temp file
    FILE *fp = fopen("CSVDataSet.ParseWithMultiplePartitionsLimit.csv", "w");
    ASSERT_TRUE(fp);
    fprintf(fp, "columnA,columnB\n");
    auto N = 50000;
    for(int i = 0; i < N; ++i) {
    ...
    size_t limit = 5;
    Context c(microTestOptions());
    auto v = c.csv("CSVDataSet.ParseWithMultiplePartitionsLimit.csv").mapColumn("columnB", UDF("lambda x: x * x")).takeAsVector(limit);
