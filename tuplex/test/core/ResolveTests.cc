...
TEST_F(Resolve, ResolveAccessesOtherCol) {
    ...
    stringToFile(URI("Resolve.ResolveAccessesOtherCol.csv"), "0,1,2,4\n1,1,2,4\n");

    auto res = c.csv("Resolve.ResolveAccessesOtherCol.csv").map(UDF("lambda x: 10 / x[0]"))
...
TEST_F(Resolve, ResolveAccessesOtherColInterpreterOnlyAndPushdown) {
    ...
    //    Row(0, 1, 2, 4), Row(1, 1, 2, 4)
    stringToFile(URI("Resolve.ResolveAccessesOtherColInterpreterOnlyAndPushdown.csv"), "0,1,2,4\n1,1,2,4\n");

    auto res = c.csv("Resolve.ResolveAccessesOtherColInterpreterOnlyAndPushdown.csv").map(UDF("lambda x: 10 / x[0]"))
            .resolve(ExceptionCode::ZERODIVISIONERROR, UDF("lambda x: x[2] * 1.0"))
            .resolve(ExceptionCode::VALUEERROR, UDF("lambda x: 0.0"))
            .collectAsVector();
