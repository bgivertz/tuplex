...
class Context:
    ...
    def parallelize(self, value_list, columns=None, schema=None, auto_unpack=True):
        ...
        assert isinstance(auto_unpack, bool) or auto_unpack is None, "auto_unpack must be given as a boolean or None"
        ...
        ds = DataSet()
        ds._dataSet = self._context.parallelize(value_list,
                                                columns,
                                                schema,
                                                auto_unpack if auto_unpack is not None else False)
        return ds
