...
namespace tuplex {
    ...
    public:
        ...
        std::shared_ptr<ResultSet> collect(std::ostream& os, bool incremental = false) override;

        // take / collect will print out the error only
        std::shared_ptr<ResultSet> take(int64_t numElements, std::ostream& os, bool incremental = false) override;

        //virtual void show(const int64_t numRows=-1, std::ostream& os=std::cout) override;
        std::vector<Row> collectAsVector(std::ostream& os, bool incremental = false) override;

        // take / collect will print out the error only
        std::vector<Row> takeAsVector(int64_t numElements, std::ostream& os, bool incremental = false) override;
    };
}
